...
export function EventDetailClient({ activity }: { activity: EventDetail }) {
  const hasDate = !!activity.date;
  const hasLocation = !!activity.location;
  const hasImpact = activity.impact && (activity.impact.partnerships || activity.impact.participants || activity.impact.outcomes || activity.impact.funding);
  const hasOverview = !!activity.overview;
  ...
  const hasTestimonial = activity.testimonial && activity.testimonial.quote;
  const hasPartners = activity.partners && activity.partners.length > 0;
                    ...
                    {activity.objectives!.map((obj, idx) => (
                      <li key={idx} className="flex items-start gap-3">
                        <div className="w-6 h-6 rounded-full bg-secondary/20 flex items-center justify-center flex-shrink-0 mt-0.5">
                          <TargetIcon className="w-4 h-4 text-secondary" />
                        </div>
                ...
                {activity.outcomes!.map((outcome, idx) => (
                  <div key={idx} className="glass-dark p-8 rounded-3xl border border-white/10 hover:border-secondary/40 transition-all duration-500">
                    <div className="flex items-center gap-3 mb-4">
                      <div className="w-10 h-10 rounded-xl bg-secondary/20 flex items-center justify-center">
                        <TrendUpIcon className="w-5 h-5 text-secondary" />
                      </div>
